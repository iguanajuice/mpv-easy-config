...
local osc_styles = {
    ...
    WindowTitle = '{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H0\\fs'.. 24 ..'\\q2\\fn' .. user_opts.font .. '}',
    Description = '{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H000000&\\fs'.. 24 ..'\\q2\\fn' .. user_opts.font .. '}',
...
function downloadDone(success, result, error)
    if success then
        show_message("\\N{\\an9}Download saved to " .. mp.command_native({"expand-path", "~~desktop/Downloads"}))
    ...
    ne.eventresponder['mbtn_left_up'] =
        function ()
            if (not state.videoCantBeDownloaded) then
                local localpathnormal = mp.command_native({"expand-path", "~~desktop/Downloads"})
